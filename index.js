...
function getThemeExtractLinkTag({ publicPath, options }) {
  let href = `/${publicPath || ''}/${options.outputDir || ''}/${options.filename}.css`.replace(/\/+(?=\/)/g, '');
  return {
    tagName: 'link',
    voidTag: true,
    attributes: {
...
class ExtractSassVarsPlugin {
  constructor(options) {
    ...
    this.options.filename = this.options.filename || 'themes';
  }
  ...
  apply(compiler) {
    ...
    compiler.hooks.emit.tapAsync(
      ...
      async (compilation, callback) => {
        let cssContent = '/* generated by extract-sass-vars-plugin */\n';
        if (this.themes.length) {
          ...
          compilation.assets[
            `${this.options.outputDir || ''}/${this.options.filename}.css`
          ] = {
            source() {
              return cssContent;
            },
